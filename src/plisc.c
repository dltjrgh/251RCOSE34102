...
PLISCRqueue *plisc_create_rqueue(int capacity) {
  PLISCRqueue *rqueue = create_heap(capacity, plisc_compare);
  if (!rqueue) {
    fprintf(stderr, "Failed to create PLISC_rqueue\n");
    return NULL;
  }
  ...
  if (!rqueue->data) {
    fprintf(stderr, "Failed to allocate memory for PLISC_rqueue data\n");
    free(rqueue);
    exit(EXIT_FAILURE);
  }
...
PLISCState *plisc_init(Pool *pool, int capacity) {
  PLISCState *state = (PLISCState *)malloc(sizeof(PLISCState));
  if (!state) {
  ...
  state->pool = pool;
  state->ready_queue = create_heap(capacity, plisc_compare);
...
void plisc_add_ready(PLISCState *state, Process *process) {
  if (!state || !process) {
    return;
  }
...
void plisc_add_waiting(PLISCState *state, Process *process) {
  if (!state || !process) {
    return;
  }
...
void plisc_add_terminated(PLISCState *state, Process *process) {
  if (!state || !process) {
    return;
  }
...
Process *plisc_remove_ready(PLISCState *state) {
...
Process *plisc_remove_waiting(PLISCState *state) {
...
void plisc_consume_time_waiting(PLISCState *state) {
  if (!state || is_empty(state->waiting_queue)) {
    return;
  }
  ...
  if (process) {
    ...
    if (workload->is_cpu == 0 && workload->duration == 0) {
      ...
      plisc_remove_waiting(state);
      if (is_empty(process->workloads)) {
        plisc_add_terminated(state, process);
      } else {
        plisc_add_ready(state, process);
      }
    }
  }
}
void plisc_consume_time_running(PLISCState *state) {
  ...
  if (workload != NULL && workload->duration > 0) {
    ...
    if (workload->duration == 0) {
      ...
      if (is_empty(state->running_process->workloads)) {
        plisc_add_terminated(state, state->running_process);
        state->running_process = NULL;
      } else {
        plisc_add_waiting(state, state->running_process);
        state->running_process = NULL;
      }
    }
  }
...
void plisc_get_from_pools(PLISCState *state) {
...
void plisc_preempt_process(PLISCState *state) {
  ...
  if (min_process &&
    ...
    add_gantt_node(state->gantt, running_process->pid, state->running_since,
                   state->current_time);
    plisc_add_ready(state, running_process);
    state->running_process = plisc_remove_ready(state);
    state->running_since = state->current_time;
  }
}

void plisc_next_step(PLISCState *state) {
  if (!state) {
    return;
  }

  plisc_get_from_pools(state);
  plisc_preempt_process(state);
  plisc_start_running(state);
  plisc_consume_time_waiting(state);
  plisc_consume_time_running(state);

  state->current_time++;
}

void plisc_print_stat(PLISCState *state) {
  if (state == NULL) {
...
void execute_plisc(PLISCState *state) {
  ...
  while (!is_empty_heap(state->ready_queue) ||
    ...
    plisc_next_step(state);
  }

  print_gantt_chart(state->gantt);
  plisc_print_stat(state);
}
