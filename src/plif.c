...
PLIFRqueue *plif_create_rqueue(int capacity) {
  PLIFRqueue *rqueue = create_heap(capacity, plif_compare);
  if (!rqueue) {
    fprintf(stderr, "Failed to create PLIF_rqueue\n");
    return NULL;
  }
  ...
  if (!rqueue->data) {
    fprintf(stderr, "Failed to allocate memory for PLIF_rqueue data\n");
    free(rqueue);
    exit(EXIT_FAILURE);
  }
...
PLIFState *plif_init(Pool *pool, int capacity) {
  PLIFState *state = (PLIFState *)malloc(sizeof(PLIFState));
  if (!state) {
  ...
  state->pool = pool;
  state->ready_queue = create_heap(capacity, plif_compare);
...
void plif_add_ready(PLIFState *state, Process *process) {
  if (!state || !process) {
    return;
  }
...
void plif_add_waiting(PLIFState *state, Process *process) {
  if (!state || !process) {
    return;
  }
...
void plif_add_terminated(PLIFState *state, Process *process) {
  if (!state || !process) {
    return;
  }
...
Process *plif_remove_ready(PLIFState *state) {
...
Process *plif_remove_waiting(PLIFState *state) {
...
void plif_consume_time_waiting(PLIFState *state) {
  if (!state || is_empty(state->waiting_queue)) {
    return;
  }
  ...
  if (process) {
    ...
    if (workload->is_cpu == 0 && workload->duration == 0) {
      ...
      plif_remove_waiting(state);
      if (is_empty(process->workloads)) {
        plif_add_terminated(state, process);
      } else {
        plif_add_ready(state, process);
      }
    }
  }
}
void plif_consume_time_running(PLIFState *state) {
  ...
  if (workload != NULL && workload->duration > 0) {
    ...
    if (workload->duration == 0) {
      ...
      if (is_empty(state->running_process->workloads)) {
        plif_add_terminated(state, state->running_process);
        state->running_process = NULL;
      } else {
        plif_add_waiting(state, state->running_process);
        state->running_process = NULL;
      }
    }
  }
...
void plif_get_from_pools(PLIFState *state) {
...
void plif_preempt_process(PLIFState *state) {
  ...
  if (min_process &&
    ...
    add_gantt_node(state->gantt, running_process->pid, state->running_since,
                   state->current_time);
    plif_add_ready(state, running_process);
    state->running_process = plif_remove_ready(state);
    state->running_since = state->current_time;
  }
}

void plif_next_step(PLIFState *state) {
  if (!state) {
    return;
  }

  plif_get_from_pools(state);
  plif_preempt_process(state);
  plif_start_running(state);
  plif_consume_time_waiting(state);
  plif_consume_time_running(state);

  state->current_time++;
}

void plif_print_stat(PLIFState *state) {
  if (state == NULL) {
...
void execute_plif(PLIFState *state) {
  ...
  while (!is_empty_heap(state->ready_queue) ||
    ...
    plif_next_step(state);
  }

  print_gantt_chart(state->gantt);
  plif_print_stat(state);
}
